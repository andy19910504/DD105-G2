...
try{
    ...
    (NULL, :route_number, $member, :event_name, now(),:event_date-2,:event_date,:meeting_place,1,:max_attendance,:event_information,:event_cover_url,1);
    ";
   
    $events = $pdo->prepare($sql);
    $events->bindValue(":route_number",$_POST["route_number"]);
    $events->bindValue(":origin_member_number",$member);
