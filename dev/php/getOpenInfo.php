...
try {
    ...
    $memNum=$_GET["number"];
    $sql = "select * FROM `event` WHERE origin_member_number=:memNum ORDER BY `event`.`event_number` DESC";
    ...
    $sql = "select a.event_number,a.origin_member_number,b.member_number,c.member_name FROM `event`a join `event_record` b on(a.event_number =b.event_number) join `member` c on(b.member_number=c.member_number) WHERE origin_member_number=:memNum;";
